...
int dijkstra(int z, const vector<edge> &E, const vector<iter> &s) {
    ...
    vector<int> f(s.size(), 0);//memset moi phan tu trong f bang 0
...
int main() {
    ...
    while (T--) {
        ...
        for (auto it=E.begin(); it != E.end(); it++) 
            if (s[it->u] == E.end()) s[it->u] = it;//neu nhu no chua co trong s -> tuc la s[it->u] la canh di tu u co trong so nho nhat	
